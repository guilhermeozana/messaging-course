        ...
        During(Pending,
            When(OrderPaid)
                .Then(context =>
                {
                    ...
                    Console.WriteLine($"Order Paid {context.Message.OrderId} of {context.Message.AmountPaid} paid");
                })
                .TransitionTo(Paid)
            ...
            When(CancelationRequested)
                .Then(context =>
                {
                    Console.WriteLine($"Cancelation requested {context.Message.OrderId}");
                    context.Saga.CanceledAt = DateTime.UtcNow;
                    context.Saga.OrderStatus = OrderStatus.CancelationRequested;
                })
                .TransitionTo(Canceled)
                .Publish(context => new OrderCanceled{OrderId = context.Saga.OrderId})
            );
        ...
        During(Paid,
            When(CancelationRequested)
                .Then(context =>
                {
                    ...
                    context.Publish(new OrderCanceled()
                    {
                        OrderId = context.Saga.OrderId
                    });
            })
            .TransitionTo(Canceled)
            .Publish(context => new RefundOrder { OrderId = context.Saga.OrderId }));
